...
for (thr in sort(unique(df$Goal_threshold))) {
  ...
  ## numeric vectors
  xi  <- as.numeric(tmp$numberofsors)
  ni  <- as.numeric(tmp$samplesize)
  lab <- paste(tmp$author)
  lab2<- paste(tmp$type_of_lipid.lowering_medication, tmp$Goal_details)
...
## 1. Load data
df <- read.csv("StatinMeta-analysis.csv",
